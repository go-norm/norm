...
// ReturningColumns creates and returns an array of ColumnFragment.
func ReturningColumns(columns ...Fragment) *Returning {
	return &Returning{ColumnsFragment: &ColumnsFragment{Columns: columns}}
}
...
func (r *Returning) Compile(layout *Template) (string, error) {
	...
	compiled, err := r.ColumnsFragment.Compile(layout)
	if err != nil {
		return "", err
	}
