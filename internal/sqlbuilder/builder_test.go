...
func TestSelect(t *testing.T) {
	t.Skip("TODO: needs mock")

	b := &sqlBuilder{t: newTemplate(&testTemplate)}
	assert := assert.New(t)

	assert.Equal(
		`SELECT DATE()`,
		b.Select(expr.Func("DATE")).String(),
	)

	assert.Equal(
		`SELECT DATE() FOR UPDATE`,
		b.Select(expr.Func("DATE")).Amend(func(query string) string {
	...
	assert.Equal(
		...
		b.Select().Distinct(expr.Raw(`ON("col1")`), "col2").From("artist").String(),
	)

	assert.Equal(
		`SELECT DISTINCT ON("col1") AS col1, "col2" FROM "artist"`,
		b.Select().Distinct(expr.Raw(`ON("col1") AS col1`)).Distinct("col2").From("artist").String(),
	)

	assert.Equal(
		`SELECT DISTINCT ON("col1") AS col1, "col2", "col3", "col4", "col5" FROM "artist"`,
		b.Select().Distinct(expr.Raw(`ON("col1") AS col1`)).Columns("col2", "col3").Distinct("col4", "col5").From("artist").String(),
	)

	assert.Equal(
		`SELECT DISTINCT ON(SELECT foo FROM bar) col1, "col2", "col3", "col4", "col5" FROM "artist"`,
		b.Select().Distinct(expr.Raw(`ON(?) col1`, expr.Raw(`SELECT foo FROM bar`))).Columns("col2", "col3").Distinct("col4", "col5").From("artist").String(),
	)

	{
		...
		assert.Equal(
			`SELECT DISTINCT ON (SELECT "foo" FROM "bar") col1, "col2", "col3", "col4", "col5" FROM "artist"`,
			b.Select().Distinct(expr.Raw(`ON ? col1`, q0)).Columns("col2", "col3").Distinct("col4", "col5").From("artist").String(),
		)
	}

	assert.Equal(
		`SELECT DISTINCT ON (SELECT foo FROM bar, SELECT baz from qux) col1, "col2", "col3", "col4", "col5" FROM "artist"`,
		b.Select().Distinct(expr.Raw(`ON ? col1`, []interface{}{expr.Raw(`SELECT foo FROM bar`), expr.Raw(`SELECT baz from qux`)})).Columns("col2", "col3").Distinct("col4", "col5").From("artist").String(),
	)

	{
		q := b.Select().
			Distinct(expr.Raw(`ON ? col1`, []*expr.RawExpr{expr.Raw(`SELECT foo FROM bar WHERE id = ?`, 1), expr.Raw(`SELECT baz from qux WHERE id = 2`)})).
			Columns("col2", "col3").
	...
	{
		rawCase := expr.Raw("CASE WHEN id IN ? THEN 0 ELSE 1 END", []int{1000, 2000})
	...
	{
		rawCase := expr.Raw("CASE WHEN id IN ? THEN 0 ELSE 1 END", []int{1000})
		sel := b.SelectFrom("artist").OrderBy(rawCase)
		assert.Equal(
	...
	{
		rawCase := expr.Raw("CASE WHEN id IN ? THEN 0 ELSE 1 END", []int{})
		sel := b.SelectFrom("artist").OrderBy(rawCase)
		assert.Equal(
	...
	{
		rawCase := expr.Raw("CASE WHEN id IN (NULL) THEN 0 ELSE 1 END")
		sel := b.SelectFrom("artist").OrderBy(rawCase)
		assert.Equal(
			`SELECT * FROM "artist" ORDER BY CASE WHEN id IN (NULL) THEN 0 ELSE 1 END`,
			sel.String(),
	...
	{
		rawCase := expr.Raw("CASE WHEN id IN (?, ?) THEN 0 ELSE 1 END", 1000, 2000)
		sel := b.SelectFrom("artist").OrderBy(rawCase)
		assert.Equal(
			`SELECT * FROM "artist" ORDER BY CASE WHEN id IN ($1, $2) THEN 0 ELSE 1 END`,
			sel.String(),
	...
	{
		sel := b.Select(expr.Func("DISTINCT", "name")).From("artist")
		assert.Equal(
	...
	assert.Equal(
		...
		b.Select().From("artist").Where(expr.Cond{1: 1}).String(),
	)

	assert.Equal(
		`SELECT * FROM "artist" WHERE (1 = ANY($1))`,
		b.Select().From("artist").Where(expr.Cond{1: expr.Func("ANY", "name")}).String(),
	)

	assert.Equal(
		`SELECT * FROM "artist" WHERE (1 = ANY(column))`,
		b.Select().From("artist").Where(expr.Cond{1: expr.Func("ANY", expr.Raw("column"))}).String(),
	)

	{
		q := b.Select().From("artist").Where(expr.Cond{"id NOT IN": []int{0, -1}})
		assert.Equal(
	...
	{
		q := b.Select().From("artist").Where(expr.Cond{"id NOT IN": []int{-1}})
		assert.Equal(
	...
	assert.Equal(
		...
		b.Select().From("artist").Where(expr.Cond{"id IN": []int{0, -1}}).String(),
	)
...

	{
		q := b.Select().From("artist").Where(
			expr.Or(
				expr.And(expr.Cond{"a": 1}, expr.Cond{"b": 2}, expr.Cond{"c": 3}),
				expr.And(expr.Cond{"d": 1}, expr.Cond{"e": 2}, expr.Cond{"f": 3}),
			),
		)
		assert.Equal(
...

	{
		q := b.Select().From("artist").Where(
			expr.Or(
				expr.And(expr.Cond{"a": 1, "b": 2, "c": 3}),
				expr.And(expr.Cond{"f": 6, "e": 5, "d": 4}),
			),
		)
		assert.Equal(
			`SELECT * FROM "artist" WHERE ((("a" = $1 AND "b" = $2 AND "c" = $3) OR ("d" = $4 AND "e" = $5 AND "f" = $6)))`,
	...
	{
		sel := b.Select().From("artist").OrderBy(expr.Raw("id = ?", 1), "name DESC")
		assert.Equal(
	...
	{
		sel := b.Select().From("artist").OrderBy(expr.Func("RAND"))
		assert.Equal(
	...
	assert.Equal(
		`SELECT * FROM "artist" ORDER BY RAND()`,
		b.Select().From("artist").OrderBy(expr.Raw("RAND()")).String(),
	)
	...
	assert.Equal(
		...
		b.SelectFrom("artist").Where(expr.Cond{"id": nil}).String(),
	)

	assert.Equal(
		`SELECT * FROM "artist" WHERE ("id" IN (NULL))`,
		b.SelectFrom("artist").Where(expr.Cond{"id": []int64{}}).String(),
	)

	assert.Equal(
		`SELECT * FROM "artist" WHERE ("id" IN ($1))`,
		b.SelectFrom("artist").Where(expr.Cond{"id": []int64{0}}).String(),
	)

	assert.Equal(
		`SELECT COUNT(*) AS total FROM "user" AS "u" JOIN (SELECT DISTINCT user_id FROM user_profile) AS up ON (u.id = up.user_id)`,
		b.Select(expr.Raw(`COUNT(*) AS total`)).From("user u").Join(expr.Raw("(SELECT DISTINCT user_id FROM user_profile) AS up")).On("u.id = up.user_id").String(),
	)

	{
		...
		assert.Equal(
			`SELECT COUNT(*) AS total FROM "user" AS "u" JOIN (SELECT DISTINCT "user_id" FROM "user_profile") AS up ON (u.id = up.user_id)`,
			b.Select(expr.Raw(`COUNT(*) AS total`)).From("user u").Join(expr.Raw("? AS up", q0)).On("u.id = up.user_id").String(),
		)
	}

	{
		...
		q1 := b.Select(expr.Raw(`COUNT(*) AS total`)).From("user u").Join(expr.Raw("? AS up", q0)).On("u.id = up.user_id AND foo = ?", 8)
	...
	assert.Equal(
		`SELECT DATE()`,
		b.Select(expr.Raw("DATE()")).String(),
	)

	{
		sel := b.Select(expr.Raw("CONCAT(?, ?)", "foo", "bar"))
		assert.Equal(
			`SELECT CONCAT($1, $2)`,
			sel.String(),
		)
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{"bar": expr.Raw("1")})
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{expr.Raw("1"): 1})
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{expr.Raw("1"): expr.Raw("1")})
		assert.Equal(
			`SELECT * FROM "foo" WHERE (1 = 1)`,
			sel.String(),
		)
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Raw("1 = 1"))
		assert.Equal(
			`SELECT * FROM "foo" WHERE (1 = 1)`,
			sel.String(),
		)
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{"bar": 1}, expr.Cond{"baz": expr.Raw("CONCAT(?, ?)", "foo", "bar")})
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{"bar": 1}, expr.Raw("? = ANY(col)", "name"))
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{"bar": 1}, expr.Cond{"name": expr.Raw("ANY(col)")})
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{"bar": 1}, expr.Cond{expr.Raw("CONCAT(?, ?)", "a", "b"): expr.Raw("ANY(col)")})
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where("bar", 2).And(expr.Cond{"baz": 1})
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").And(expr.Cond{"bar": 1})
		assert.Equal(
			`SELECT * FROM "foo" WHERE ("bar" = $1)`,
			sel.String(),
		)
	...
	{
		sel := b.SelectFrom("foo").Where("bar", 2).Where(expr.Cond{"baz": 1})
		assert.Equal(
			`SELECT * FROM "foo" WHERE ("bar" = $1 AND "baz" = $2)`,
			sel.String(),
		)
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Raw("bar->'baz' = ?", true))
		assert.Equal(
	...
	{
		sel := b.SelectFrom("foo").Where(expr.Cond{}).And(expr.Cond{})
		assert.Equal(
			`SELECT * FROM "foo"`,
			sel.String(),
		)
	...
	{
		sel := b.SelectFrom("foo").Where("bar = 1").And(expr.Or(
			expr.Raw("fieldA ILIKE ?", `%a%`),
			expr.Raw("fieldB ILIKE ?", `%b%`),
		))
		assert.Equal(
...

	{
		...
		sel := b.Select("c1").Columns(expr.Raw(s, []int{5, 4, 3, 2})).From("foo").Where("bar = ?", 1)
		assert.Equal(
...

	{
		s := `SUM(CASE WHEN foo in ? THEN 1 ELSE 0 END) AS _sum`
		sel := b.Select("c1").Columns(expr.Raw(s, []int{5, 4, 3, 2})).From("foo").Where("bar = ?", 1)
		sel2 := b.SelectFrom(sel).As("subquery").Where(expr.Cond{"foo": "bar"}).OrderBy("subquery.seq")
		assert.Equal(
	...
	{
		series := b.Select(
			expr.Raw("start + interval ? - interval '1s' AS end", "1 day"),
		).From(
			b.Select(
				expr.Raw("generate_series(?::timestamp, ?::timestamp, ?::interval) AS start", 1, 2, 3),
			),
		).As("series")
		...
		distinct := b.Select().Distinct(
			expr.Raw(`ON(dt.email) SUBSTRING(email,(POSITION('@' in email) + 1),252) AS email_domain`),
			"dt.event_type AS event_type",
			expr.Raw("count(dt.*) AS count"),
...

	{
		sq := b.
			Select("user_id").
			From("user_access").
			Where(expr.Cond{"hub_id": 3})

		// Don't reassign
		_ = sq.And(expr.Cond{"role": []int{1, 2}})
		...
		// Reassign
		sq = sq.And(expr.Cond{"role": []int{1, 2}})
		...
		cond = cond.Or(expr.Cond{"ml.mailing_list_id": []int{4, 5, 6}})

		sel := b.
			Select(expr.Raw("DISTINCT ON(a.id) a.id"), expr.Raw("COALESCE(NULLIF(ml.name,''), a.name) as name"), "a.email").
		...
		search := "word"
		sel = sel.And(expr.Or(
			expr.Raw("COALESCE(NULLIF(ml.name,''), a.name) ILIKE ?", fmt.Sprintf("%%%s%%", search)),
			expr.Cond{"a.email ILIKE": fmt.Sprintf("%%%s%%", search)},
		))
...
func TestUpdate(t *testing.T) {
	...
	{
		idSlice := []int64{8, 7, 6}
		q := b.Update("artist").Set(expr.Cond{"some_column": 10}).Where(expr.Cond{"id": 1}, expr.Cond{"another_val": idSlice})
		assert.Equal(
	...
	{
		idSlice := []int64{}
		q := b.Update("artist").Where(expr.Cond{"id": 1}, expr.Cond{"another_val": idSlice}).Set(expr.Cond{"some_column": 10})
		assert.Equal(
			`UPDATE "artist" SET "some_column" = $1 WHERE ("id" = $2 AND "another_val" IN (NULL))`,
			q.String(),
		)
	...
	assert.Equal(
		`UPDATE "artist" SET "name" = $1 WHERE ("id" < $2)`,
		b.Update("artist").Set(map[string]string{"name": "Artist"}).Where(expr.Cond{"id <": 5}).String(),
	)
	...
	assert.Equal(
		...
		}{"Artist"}).Where(expr.Cond{"id <": 5}).String(),
	)

	assert.Equal(
		`UPDATE "artist" SET "name" = $1 WHERE ("id" < $2)`,
		b.Update("artist").Where(expr.Cond{"id <": 5}).Set(struct {
			Nombre string `db:"name"`
		}{"Artist"}).String(),
	)
	...
	assert.Equal(
		...
		b.Update("artist").Set(struct {
			Nombre string `db:"name"`
		}{"Artist"}).Set(map[string]string{"last_name": "Foo"}).Where(expr.Cond{"id <": 5}).String(),
	)
	...
	{
		q := b.Update("posts").Set(expr.Raw("column = ?", "foo"))
...

	{
		q := b.Update("posts").Set(
			expr.Cond{"tags": expr.Raw("array_remove(tags, ?)", "foo")},
		).Where(expr.Raw("hub_id = ? AND ? = ANY(tags) AND ? = ANY(tags)", 1, "bar", "baz"))
...
func BenchmarkUpdate4(b *testing.B) {
	bt := WithTemplate(&testTemplate)
	for n := 0; n < b.N; n++ {
		_ = bt.Update("artist").Set(map[string]string{"name": "Artist"}).Where(expr.Cond{"id <": 5}).String()
	}
}
...
func stripWhitespace(in string) string {
	q := exql.InvisibleCharsRegexp.ReplaceAllString(in, ` `)
