...
func (b *BatchInserter) Values(values ...interface{}) norm.BatchInserter {
	b.values <- values
	return b
}
...
func (b *BatchInserter) NextResult(ctx context.Context, dst interface{}) bool {
	clone := b.nextQuery()
	if clone == nil {
		return false
	}
	b.err = clone.Iterator(ctx).All(ctx, dst)
	return b.err == nil
}
...
func (b *BatchInserter) Wait(ctx context.Context) error {
	for {
		q := b.nextQuery()
		if q == nil {
			break
