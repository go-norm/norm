...
const (
  ...
  `
    ...
    UPDATE
      {{.Table | compile}}
    SET {{.ColumnValuesFragment | compile}}
      {{.WhereFragment | compile}}
  `
...
func newTemplate() *exql.Template {
	...
	return &exql.Template{
		...
		ops: map[expr.ComparisonOperator]string{
